-- Supabase Performance Optimization for Vail Scavenger Hunt
-- Execute this AFTER the main schema and RLS setup

-- Additional performance indexes for common query patterns
-- Team lookup optimizations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_teams_org_hunt_name
  ON teams(organization_id, hunt_id, name);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_teams_display_name_gin
  ON teams USING gin(to_tsvector('english', display_name));

-- Progress tracking optimizations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hunt_progress_team_done_location
  ON hunt_progress(team_id, done, location_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hunt_progress_completed_at
  ON hunt_progress(completed_at DESC) WHERE done = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hunt_progress_team_completed
  ON hunt_progress(team_id, completed_at DESC) WHERE done = true;

-- Session management optimizations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_team_created
  ON sessions(team_id, created_at DESC) WHERE is_active = true;

-- Team code lookup optimizations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_team_codes_org_hunt_active
  ON team_codes(organization_id, hunt_id, is_active) WHERE is_active = true;

-- Settings lookup optimizations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_settings_team_org_hunt
  ON settings(team_id, organization_id, hunt_id);

-- Leaderboard optimization indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_teams_score_completion
...
CREATE INDEX idx_leaderboard_cache_org_hunt_rank
  ON leaderboard_cache(organization_id, hunt_id, rank);

CREATE INDEX idx_leaderboard_cache_updated
...
-- Create triggers for automatic leaderboard cache refresh
CREATE OR REPLACE FUNCTION trigger_leaderboard_refresh()
RETURNS TRIGGER AS $$
BEGIN
  -- Use pg_notify to trigger cache refresh asynchronously
  PERFORM pg_notify('leaderboard_refresh',
    json_build_object(
      'organization_id', COALESCE(NEW.organization_id, OLD.organization_id),
      'hunt_id', COALESCE(NEW.hunt_id, OLD.hunt_id),
      'team_id', COALESCE(NEW.team_id, NEW.id, OLD.team_id, OLD.id)
    )::text
  );
...
-- Attach triggers to tables that affect leaderboard
CREATE TRIGGER trigger_leaderboard_refresh_teams
  AFTER INSERT OR UPDATE OR DELETE ON teams
  FOR EACH ROW EXECUTE FUNCTION trigger_leaderboard_refresh();

CREATE TRIGGER trigger_leaderboard_refresh_progress
  AFTER INSERT OR UPDATE OR DELETE ON hunt_progress
  FOR EACH ROW EXECUTE FUNCTION trigger_leaderboard_refresh();
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hunt_progress_incomplete
  ON hunt_progress(team_id, location_id, created_at) WHERE done = false;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_teams_active_hunts
  ON teams(organization_id, hunt_id, score DESC)
  WHERE id IN (
    SELECT DISTINCT team_id FROM sessions WHERE is_active = true
  );
