-- Supabase Database Schema for Vail Scavenger Hunt (Safe Version)
-- Execute this in the Supabase SQL Editor - handles existing tables
...
CREATE INDEX IF NOT EXISTS idx_progress_team ON hunt_progress(team_id);
CREATE INDEX IF NOT EXISTS idx_sessions_expires ON sessions(expires_at) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_team_codes_active ON team_codes(code) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_hunt_progress_location ON hunt_progress(location_id);
...
-- Insert sample data (safe - will skip if exists)
...
-- Create trigger (safe)
DROP TRIGGER IF EXISTS trigger_update_team_score ON hunt_progress;
...
-- Add constraints (safe - will skip if exists)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_revealed_hints') THEN
    ALTER TABLE hunt_progress ADD CONSTRAINT check_revealed_hints CHECK (revealed_hints >= 0);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_score_non_negative') THEN
    ALTER TABLE teams ADD CONSTRAINT check_score_non_negative CHECK (score >= 0);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_usage_count_non_negative') THEN
    ALTER TABLE team_codes ADD CONSTRAINT check_usage_count_non_negative CHECK (usage_count >= 0);
  END IF;
END $$;

-- Additional indexes
CREATE INDEX IF NOT EXISTS idx_hunt_progress_done ON hunt_progress(done);
